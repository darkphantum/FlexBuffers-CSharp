...
{
	public class JsonToFlexBufferFastConverter
	{
		private readonly TextScanner _scanner;

		private JsonToFlexBufferFastConverter(TextReader reader)
		{
		...
		public static byte[] Convert(TextReader reader, FlexBuffer.Options options = FlexBuffer.Options.ShareKeys | FlexBuffer.Options.ShareStrings | FlexBuffer.Options.ShareKeyVectors)
		{
			...
			new JsonToFlexBufferFastConverter(reader).ReadJsonValue(flx);
			return flx.Finish();
		}
		...
		private void ReadString(FlexBuffer flx, bool asKey = false)
		{
				...
				if (c == '\\')
				{
					...
					switch (char.ToLower(c))
					{
						...
						case 't':
							...
							break;
						case 'u':
							builder.Add((byte)ReadUnicodeLiteral());
							break;
						default:
							throw new Exception($"Unexpected character {_scanner.Position}");
					}
			...
			if (asKey)
			{
				flx.AddKey(builder.ToArray());
			}
			else
			{
				flx.AddFlexStringBlob(builder.ToArray());
			}
		}
